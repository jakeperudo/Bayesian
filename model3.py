...
init = np.array([66500000, 46000, 2400, 0])

def f(y, t0, beta1, beta2, gamma1, gamma2, delta, echelon, alpha):
    S, IA, IS, H = y
    dS = - (S*beta1*IA) - (S*beta2*IS)
    dIA = (S*beta1*IA) + (S*beta2*IS) - (gamma1*IA) - (delta*IA)
    dIS = (delta*IA) - (gamma2*IS) - (echelon*IS)
    dH = (alpha*IS)
    return dS, dIA, dIS, dH


def model(pars):
    sol = sp.integrate.odeint(
             f, init, measurement_times,
             args=(pars["beta1"], pars["beta2"], pars["gamma1"], pars["gamma2"], pars["delta"], pars["echelon"], pars["alpha"]))
